...
**응답 422** — `position`이 1~5 밖이거나, 글이 비었거나 **50자**를 넘을 때 (앞뒤 공백은 저절로 잘려요)
...
```json
{
  ...
  "pending_received": [
    { "friendship_id": 7, "note_id": "hajun9", "nickname": "하준", "avatar": "cat" }
  ],
  "max_friends": 10
}
```

> `max_friends`를 같이 내려줘요. 화면의 **"내 친구 4 / 10"** 에서 10을 프론트에 또 적어두면
> 나중에 상한을 바꿀 때 두 군데를 고쳐야 하거든요.
> `friends`는 별명 순서예요. 내가 신청한 친구와 받은 친구를 **합쳐서** 줍니다.

### 4-3. 친구 신청 — `POST /friends`
요청: `{ "note_id": "minjun22" }` → 응답 201 `{ "friendship_id": 7, "status": "pending", "note_id": …, "nickname": …, "avatar": … }`

| 코드 | 언제 |
|---|---|
| 404 | 그런 수첩 아이디가 없을 때 |
| 400 | 나에게 신청했을 때 |
| 409 | 이미 친구이거나 신청함 (**반대 방향으로 신청한 경우도** 포함) |
| 409 | `{ "detail": "친구는 10명까지 사귈 수 있어요" }` — 내 자리가 꽉 참 (D-22) |
| 409 | `{ "detail": "그 친구는 이미 친구가 가득 찼어요" }` — 상대 자리가 꽉 참 |

### 4-4. 수락 — `POST /friends/{friendship_id}/accept`
**받은 사람만** 수락할 수 있어요. 신청한 사람이 스스로 수락하면 상대 뜻과 상관없이 친구가 되니까요.

| 코드 | 언제 |
|---|---|
| 200 | 수락됨 — `{ "friendship_id": 7, "status": "accepted", … }` |
| 404 | 그런 신청이 없거나 **내가 받은 신청이 아닐 때** (403이라고 하면 "그 번호는 있구나"를 알려주는 셈이라 404로 숨겨요) |
| 409 | 이미 친구 |
| 409 | 상한 초과 — 수락 시점엔 **양쪽 다** 검사해요 (D-22). 신청할 땐 자리가 있었어도 그 사이에 찰 수 있거든요 |

### 4-5. 거절·삭제 — `DELETE /friends/{friendship_id}`

**응답 204** (본문 없음). 신청한 쪽·받은 쪽 **둘 다** 지울 수 있어요 — 거절·취소·친구 끊기가 전부 이 하나예요.
**404** — 내가 낀 관계가 아닐 때
...
친구들의 **오늘** 수첩. 오늘 아직 시작 안 한 친구는 안 나와요 (누가 내 친구인지는 4-2가 알려줘요).
  ...
  "feed": [
    {
      ...
      "status": "complete",
      "progress": "5/5",
      "written_at": "2026-08-26T14:05:31",
...
> 🔒 친구가 아닌 사람의 수첩을 부르면 **403**을 돌려줘요. 4-6·4-7·4-8·4-9 전부요.

**구현하면서 명세와 달라진 것 3가지**

| 항목 | 명세 | 실제 | 왜 |
|---|---|---|---|
| `country` | `"KR"` | **뺐어요** | `cat_users`에 나라 컬럼이 없어요. 배우는 언어(`ko`)로 나라를 짐작하면 틀려요 — 한국어를 배우는 사람이 다 한국에 살진 않으니까요. 국기를 보여주려면 컬럼을 새로 만들어야 해요 |
| `written_at` | `"10분 전"` | ISO 시각 | 앱이 4개 언어예요 (D-08). 서버가 "10분 전"을 만들면 번역까지 서버 몫이 돼요. 시각만 주고 문구는 화면에서 만들어요 (D-25) |
| `status` | `"complete"` | `"complete"` / `"writing"` | 쓰는 중인 친구도 보여줘야 해서 값을 하나 더 정했어요 |

> 📖 `sentences`는 **친구가 쓴 그대로**예요. 교정본이 아니라요.
> 친구에게 "너 여기 틀렸대" 를 보여주는 화면이 아니거든요.

> ~~어른 모드는 국기·배우는 언어도 같이~~ → D-16으로 모드 구분이 없어졌어요.

### 4-7. 칭찬도장 주기 — `POST /entries/{entry_id}/praises`
응답 201 `{ "praise_count": 4 }`

| 코드 | 언제 |
|---|---|
| 409 | 이미 준 경우 (한 수첩에 한 사람이 하나) |
| 403 | **내 수첩** — `{ "detail": "내 수첩에는 칭찬도장을 못 줘요" }` |
| 403 | 친구가 아닌 사람의 수첩 |
| 404 | 그런 수첩이 없을 때 |

### 4-8. 댓글 목록 — `GET /entries/{entry_id}/comments`

**친구 수첩과 내 수첩 둘 다** 볼 수 있어요. 내 글에 달린 칭찬을 못 보면 댓글이 무슨 소용이겠어요.
  ...
  "comments": [
    {
      ...
      "avatar": "cat",
      "content": "잘 썼다!",
      "created_at": "2026-08-26T14:05:31"
    }
  ]
}
```
...
요청: `{ "content": "잘 썼다!" }` → 응답 201 (4-8의 댓글 한 개 모양)

**422** — 비었거나 200자를 넘을 때 (앞뒤 공백은 저절로 잘려요)
**403** — 친구도 내 것도 아닌 수첩
...
> ~~(어른 모드)~~ → **D-16으로 화면이 하나라 모두가 써요.**
> 여기 모은 개수가 **내 단계를 정해요** (D-23) — 50개마다 한 단계.
  ...
  "vocab": [
    {
      "vocab_id": 12,
      "expression": "좋아요",
      "meaning": "'좋다'의 어간은 좋-이라서 ㅎ 받침을 유지해요.",
      "correction_id": 5,
      "created_at": "2026-08-26T14:05:31"
    }
  ]
...
요청: `{ "correction_id": 5 }` → 응답 201 (5-1의 항목 한 개 모양)

**교정에서만 담을 수 있어요.** 아무 말이나 적어 넣는 메모장이 아니라
"내가 오늘 틀려서 배운 것"을 모으는 자리거든요.
`correction_id`는 채점 응답(2-3)과 지난 수첩(3-2)의 `corrections` 안에 들어 있어요.

| 코드 | 언제 |
|---|---|
| 404 | 그런 교정이 없을 때 |
| 403 | **내 수첩에서 나온 교정이 아닐 때** — 친구 글에서 배운 표현이라도 몰래 담을 순 없어요 |
| 409 | 이미 단어장에 있을 때 |

> `meaning`은 컬럼이 200자라 문법 노트가 길면 잘려요.
> 전체 설명은 그날 수첩(3-2)을 열면 그대로 남아 있어요.

### 5-3. 빼기 — `DELETE /vocab/{vocab_id}`

**응답 204** (본문 없음) · **404** 내 단어장에 없는 번호일 때
...
- [ ] 짝꿍 말투를 **어디서 처리할지** — 프론트에 4종 문구를 넣을지, 서버가 말투에 맞춰 내려줄지
      ...
      참고: 시각 문구("10분 전")는 **프론트에서 만들기로** 이미 정했어요 (D-25). 같은 결로 가면 될 것 같아요
